...
class Bottleneck4Sequence(nn.Module):
	expansion = 1.5
	def __init__(self, in_channel, out_channel, stride=1, downsample=None):
		super(Bottleneck4Sequence, self).__init__()
...
class ResNet4Sequence(nn.Module):
	def __init__(self, block, layers, num_letter=4, num_classes=2, first_out_channel=20):
		super(ResNet4Sequence, self).__init__()
		self.chan1 = 20

		# first block
		self.first_layer = nn.Sequential(
			nn.Conv1d(num_letter, 20, 19, padding=5, stride=3),
			bcnorm(self.chan1),
		...
		self.layer1 = self._make_layer(block, first_out_channel, layers[0])
